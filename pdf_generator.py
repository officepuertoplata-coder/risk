...
# ── Software Technologies Farbpalette (hell) ──────────────────
INK    = (15,  30,  51)
INKSOFT= (71,  85,  105)
MGRAY  = (100, 116, 139)
DIM    = (148, 163, 184)
...
DIM_COLORS = {"legal": BLUE, "cyber": PURPLE, "operational": ORANGE, "financial": GREEN}
...
class SwtechPDF(FPDF):
    ...
    def header(self):
        ...
        self.set_xy(16, 6)
        self.cell(90, 10, "Software Technologies")
    ...
    def footer(self):
        ...
        self.cell(0, 8, f"Software Technologies - SSOT Supplier Risk Management  -  {datetime.now().strftime('%d.%m.%Y')}  -  Vertraulich  -  Seite {self.page_no()}  -  powered by Ynhald", align="C")

    def section_title(self, title, color=CYAN):
...
def generate_pdf(data: Dict[str, Any]) -> bytes:
    ...
    for i, (lbl, val) in enumerate(meta):
        ...
        pdf.set_fill_color(*(CYAN if i < 2 else BLUE))
        pdf.rect(x, y, 3, 12, "F")
        pdf.set_xy(x + 5, y + 1)
    ...
    pdf.section_title("Executive Summary", CYAN)
    exec_text = safe(analysis.get("exec", ""))
    pdf.set_fill_color(*PANEL)
    ey = pdf.get_y()
    pdf.set_font("Helvetica", "", 8.5)
    lines = pdf.multi_cell(164, 4.5, exec_text, dry_run=True, output="LINES")
    box_h = max(14, len(lines) * 4.5 + 6)
    pdf.rect(16, ey, 178, box_h, "F")
    pdf.set_fill_color(*CYAN)
    pdf.rect(16, ey, 4, box_h, "F")
    pdf.set_xy(24, ey + 3)
    pdf.set_text_color(*INK)
    pdf.multi_cell(164, 4.5, exec_text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(4)

    pdf.section_title("Risikodimensionen", CYAN)
    ...
    i_lines = pdf.multi_cell(166, 4.5, impact_text, dry_run=True, output="LINES")
    ...
    pdf.section_title("Sofortmassnahmen", INK)
    prio_colors = [CYAN, BLUE, LBLUE]
    prio_bgs    = [(235,250,248), (240,244,255), (236,247,255)]
    ...
    pdf.rect(16, cta_y, 178, 24, "FD")
    pdf.set_line_width(0.2)
    pdf.set_fill_color(*CYAN)
    pdf.rect(16, cta_y, 4, 24, "F")
    ...
    pdf.set_x(24)
    pdf.set_font("Helvetica", "", 8.5)
    pdf.set_text_color(*INKSOFT)
    pdf.cell(0, 6, "Kostenloses Erstgespraech: office@sw-tech.net  |  cal.com/alexander-zajic/riskmanagement")
    pdf.ln(14)
    ...
    for dim, dim_label in DIM_LABELS.items():
        ...
        for q in dim_qs:
            ...
            ans_clean = (ans.replace("Weiß nicht","Weiss nicht").replace("Don't know","Weiss nicht"))
    ...
    pdf.section_title("Paketempfehlung", CYAN)
    pr_y = pdf.get_y()
    pkg_name = safe(analysis.get("pkg", "Guided Remediation"))
    pkg_why  = safe(analysis.get("pkgWhy", ""))
    why_lines = pdf.multi_cell(170, 4.5, pkg_why, dry_run=True, output="LINES")
    ...
    pdf.check_page_break(46)
    fta_y = pdf.get_y()
    pdf.set_fill_color(*PANEL)
    pdf.set_draw_color(*CYAN)
    pdf.set_line_width(0.4)
    pdf.rect(16, fta_y, 178, 42, "FD")
    pdf.set_line_width(0.2)
    pdf.set_fill_color(*CYAN)
    pdf.rect(16, fta_y, 4, 42, "F")
    pdf.set_xy(24, fta_y + 5)
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(*INK)
    pdf.cell(0, 7, "Ihr Ansprechpartner", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(24)
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(*INK)
    pdf.cell(0, 6, "Alexander Zajic  -  Software Technologies", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(24)
    pdf.set_font("Helvetica", "B", 9)
    pdf.set_text_color(*CYAN)
    pdf.cell(0, 5, "Tel. +43 676 84949610   |   office@sw-tech.net", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(24)
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(*MGRAY)
    pdf.cell(0, 5, "Kostenloses Erstgespraech: cal.com/alexander-zajic/riskmanagement", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_x(24)
    pdf.cell(0, 5, "Software Technologies - SSOT Supplier Risk Management")
...
if __name__ == "__main__":
    data = {
        ...
        "industryLabel": "Kosmetik",
        "answers": {q["id"]: ("Ja" if i % 3 else "Nein") for i, q in enumerate(QUESTIONS)},
        "scores": {"color": "yellow", "final": 62, "ds": {"legal": 80, "cyber": 45, "operational": 70, "financial": 55}},
        "analysis": {"exec": "Ihr Lieferantenrisiko ist mittel. Zwei Bereiche brauchen Aufmerksamkeit.",
                     "impact": "Bei Ausfall drohen Lieferverzoegerungen und Compliance-Luecken.",
                     "p1": {"title": "Notfallplan erstellen", "text": "Business-Continuity-Plan aufsetzen und testen."},
                     "p2": {"title": "Audits einfuehren", "text": "Jaehrliche Audits fuer Tier-1-Lieferanten."},
                     "p3": {"title": "Monitoring", "text": "Laufende Ueberwachung der Kennzahlen."},
                     "pkg": "Professional", "pkgWhy": "Passt zu Ihrer Unternehmensgroesse und Risikolage."}
    }
    open("test_assessment.pdf", "wb").write(generate_pdf(data))
    print("Assessment-PDF erzeugt")
